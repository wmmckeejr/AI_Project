...
This project will use an object detection model using Yolo (1) and a yaml dataset (yaml is a special format used by YOLO) from Roboflow (2) to capture food items that are to be used for a recipe.  Once the items have been identified, they will be provided to a large language model from The Meal DB website (3) or Chat GPT (4) to generate an appropriate recipe using those ingredients.  The recipe wll then be displayed for the user to enjoy.  I later commented out the Meal DB (3)version due to an issue calling their API on my computer even though it works fine from Google Colab (6).
